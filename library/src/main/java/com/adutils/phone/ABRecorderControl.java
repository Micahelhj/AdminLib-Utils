...
/**
 * 项目名称：AdminLibs
 * 类描述： ABRecorderControl 录音
 * 创建时间：2016/2/4 14:45
 * 修改人：Michael
 * 修改时间：2016/2/4 14:45
 * 修改备注：
 */
public class ABRecorderControl {
    ...
    public ABRecorderControl() {

    }

    public ABRecorderControl(String name) {
        if (mRecorder == null) {
    ...
    public void playingFinish() {
        Log.i("spoort_list", "ABRecorderControl 播放结束释放资源");
        if (playState) {
            playState = false;
        }
        mPlayer.release();
    ...
    public boolean stopPlaying() {
        ...
        if (mPlayer != null) {
            ...
            Log.i("spoort_list", "ABRecorderControl mPlayer.stop()");
            mPlayer.stop();
            mPlayer.release();
            mPlayer = null;
            playState = false;
            ...
            Log.i("spoort_list", "ABRecorderControl mPlayer.stop() is null");
            return false;
        }
    }
    //当文件播放结束后调用此方法
    ...
    ////				playState = false;
    ////			}
    //			Log.i("spoort_list", "ABRecorderControl 播放结束");
    //			mPlayer.release();
    //			mPlayer = null;
    //		}
    //	};
