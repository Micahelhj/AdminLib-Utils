 ...
 * 修改备注：
 */
public class ABLogRecordUtil {
    private static final String TAG = "ABLogRecordUtil";
    private static final SimpleDateFormat timeFormat = new SimpleDateFormat("MM-dd HH:mm:ss.SSS", Locale.getDefault());//每条日志的时间
    private static final HashMap<String, SoftReference<BlockingQueue<String>>> queueHashMap = new HashMap<>();//日志保存休息对象集合
    private static final HashMap<String, SoftReference<PoolWorker>> poolWorkerHashMap = new HashMap<>();//日志保存线程对象
    ...
    private static ABLogRecordUtil instance;//日志写入工具单例对象

    private ABLogRecordUtil(final String[] logFileNames) {
            ...
            public void run() {
                try {
                    for (String name : logFileNames) {
                        ...
                        String fileName = name + "_" + "_" + System.currentTimeMillis() + ".log";//日志文件名
    ...
    public static void createInstance(String[] logFileNames) {
        instance = new ABLogRecordUtil(logFileNames);
    }
     ...
     * @return 日志记录管理器单例
     */
    public static ABLogRecordUtil getInstance() {
        return instance;
    }
    ...
    public void recordLog(String logTag, String data) {
        ...
        synchronized (logTag) {
            //日志信息放入队列时间超过200毫秒判定放入失败
            if (queueHashMap.get(logTag) == null || queueHashMap.get(logTag).get() == null) {
                queueHashMap.put(logTag, new SoftReference<BlockingQueue<String>>(new LinkedBlockingDeque<String>()));//创建消息队列
            }
            SoftReference<PoolWorker> workerSoftReference = poolWorkerHashMap.get(logTag);
            if (!queueHashMap.get(logTag).get().offer(data)) {
                Log.e(TAG, "<" + logTag + ">日志进入队列失败");
            } else if (workerSoftReference == null || workerSoftReference.get() == null) {//如果线程不在线程池则创建线程并放入线程池
                workerSoftReference = new SoftReference<>(new PoolWorker(logTag));//创建日志写入线程
                poolWorkerHashMap.put(logTag, workerSoftReference);//将线程放入集合
                pool.execute(workerSoftReference.get());//将线程放入线程池
            } else if (!workerSoftReference.get().isRunning) {//如果线程不在运行状态则 启动线程
                workerSoftReference.get().run();
            }
        }
    }
    ...
    private class PoolWorker implements Runnable {
        ...
        public PoolWorker(String logTag) {
            this.logTag = logTag;
            this.queue = queueHashMap.get(logTag).get();
        }

        public void run() {
            this.isRunning = true;
            while (isRunning) {
                String data = queue.poll();//从消息队列取出第一条消息（如果没有消息也不进行阻塞）
                if (null != data) {
                    writeLogDataToFile(logTag, data);
     ...
     * @param content 上下文
     */
    public void writeLogDataToFile(String logTag, String content) {
        try {// 进行写操作
            ...
            if (bufferedWriter != null) {
                ...
                System.out.println("logTag === " + logTag + "  file == null || bufferedWriter == null");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
